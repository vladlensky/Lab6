...
public class Interface extends NotificationBroadcasterSupport implements InterfaceMBean{
    private MbeanClass mbean;
    private double procent = 0;
    private int all = 0;
    private int numberOfTrueStrikes = 0;
    private int numberOfFalseStrikes = 0;
    private static Locale locale = new Locale("ru","RU");
    public static void setLocale(Locale locale) {Interface.locale = locale;}
    ...
    public static int sequensNumber = 1;
    private static CloseFrame cf = new CloseFrame(bwc);
    public void send(){
        this.sendNotification(new Notification("Added 5 moves", this, sequensNumber++, System.currentTimeMillis(), "Next five!"));
    }
    private void FalseStrike(){numberOfFalseStrikes+=1;
        all++;
        if(all%5==0)
            send();
        procent=(double)numberOfTrueStrikes/all*100;
    }
    private void TrueStrike(){numberOfTrueStrikes+=1;
        all++;
        if(all%5==0)
            send();
        procent=(double)numberOfTrueStrikes/all*100;
    ...
    public int getNumberOfTrueStrikes(){return numberOfTrueStrikes;}
    public int getNumberOfFalseStrikes() {return numberOfFalseStrikes;}
    ...
    Interface(){
        ...
        collections.addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                if(e.getKeyCode()==KeyEvent.VK_DELETE) but.delete();
            }
        });
    ...
    public static void main(String[] args){
        ...
        if(!er){
            ...
            SwingUtilities.invokeLater(()-> new MbeanClass());
        }
    }
}